...
export function ProfilePage({
  ...
  // Group answers by question key (skip allergies_other, diet_notes if empty)
  const answers = profile?.answers ?? [];
  const allergyAnswers = answers
    .filter((a) => a.question.key === "allergies" && a.option)
    .map((a) => a.option!.label);
  const dietAnswers = answers
    .filter((a) => a.question.key === "diet" && a.option)
    .map((a) => a.option!.label);
  const dislikedText = answers.find((a) => a.question.key === "disliked_ingredients")?.answerText;
  const dietNotesText = answers.find((a) => a.question.key === "diet_notes")?.answerText;
  const allergiesOther = answers.find((a) => a.question.key === "allergies_other")?.answerText;
